...
export async function validateProof(file:File){if(!file.size||file.size>MAX_PROOF_BYTES)throw Error('Choose a PDF, PNG or JPEG no larger than 5 MB.');proofType(new Uint8Array(await file.slice(0,8).arrayBuffer()));}
